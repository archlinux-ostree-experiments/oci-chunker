...
impl BuildChunkedOCIOpts {
    pub(crate) fn run(self) -> Result<OstreeRepoFromContainerResult> {
        ...
        let args: [String; 11] = [
                ...
                repo_path.to_string(),
                label_arg.join(" "),
                self.max_layers.map(|l| format!("--max-layers={l}")).unwrap_or(String::from("--max-layers=128")),
                format!("--arch={arch}"),
                config_data
                    .iter()
                    ...
                    .unwrap_or(Vec::new())
                    .join(" "),
                commitid.clone(), self.output,
                manifest_data.as_ref().iter().flat_map(|manifest| {
                    [
    ...
    fn check_existing_image(&self, output: &str) -> Result<Option<oci_spec::image::ImageManifest>> {
        ...
        let result: Option<oci_spec::image::ImageManifest> = handle.block_on(async {
            ...
            let img = proxy.open_image_optional(output)
                .await
